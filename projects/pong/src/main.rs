...
pub struct App {
    ...
    // left paddle with velocity params
    left_score: i32,
    left_pos: i32,
    left_vel: i32,

    // right paddle with velocity params
    right_score: i32,
    right_pos: i32,
    right_vel: i32,

    // the 'ball' itself with velocity params
    ball_x: i32,
    ball_y: i32,
    vel_x: i32,
    vel_y: i32,
...
impl App {
    fn render(&mut self, args: &RenderArgs) {
        ...
        const FOREGROUND_PADDLE: [f32; 4] = [0.0, 0.0, 1.0, 1.0];
        const FOREGROUND_BALL: [f32; 4] = [1.5, 0.0, 0.0, 1.0];
        ...
        self.gl.draw(args.viewport(), |c, gl| {
            clear(BACKGROUND, gl);
            rectangle(FOREGROUND_PADDLE, left, c.transform.trans(-40.0, left_pos), gl);
            // RenderArgs fields:
            // ext_dt: f64
            ...
            rectangle(FOREGROUND_PADDLE, right, c.transform.trans(args.window_size[0] as f64 - 10.0, right_pos), gl);
            rectangle(FOREGROUND_BALL, ball, c.transform.trans(ball_x, ball_y), gl);
        });
    }
...
fn main() {
    ...
    let mut window: GlutinWindow = WindowSettings::new("Pong", [512, 342]).exit_on_esc(true).build().unwrap();
